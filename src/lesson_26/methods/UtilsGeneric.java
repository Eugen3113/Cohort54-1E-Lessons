...
public class UtilsGeneric {
    ...
    public static <T> String printArray(T[] array) {
        ...
        for (int i = 0; i < array.length; i++) {
            sb.append(array[i]);
            sb.append(i < array.length - 1 ? ", " : "]"); // проверяем если i меньше чем последний индекс -1, то добавляем ", " ,в противном случае "]" .
        }
    ...
    public static <T> void swap(T[] array, int idx1, int idx2) {
        T temp = array[idx1];  // записываем во временную перем-ю temp, значение idx1
        array[idx1] = array[idx2]; // в пустую ячейку [idx1],записываем знач-е [idx2]
        array[idx2] = temp;        // в array[idx2] записываем значение из temp.
    }                 // array[idx1] -> temp, array[idx2] -> array[idx1], temp -> array[idx2].
