 ...
 */
    ...
    Композиция - это более жесткая форма агрегации, где часть не может существовать юез целого
     ...
     Для создания связи типа HAS_A мы определяем поле класса,
...
public class Autobus {
    ...
    private BusDriver driver; // агрегация (мягкая связь) // ссылка на объект приходит извне
    private Autopilot autopilot; // композиция (жесткая / неразрывная связь)  // сами создаём объект внутри конструктора
    ...
    public Autobus(BusDriver busDriver, int capacity) {
        ...
        // Часть (объект автопилота (строка:62 -> 75)) создается при создании целого (объект автобуса) :
        this.autopilot = new Autopilot("AP-v001"); // если нет автобуса- тогда нет автопилота
                                                     // композиция (жесткая) // сами создаём объект внутри конструктора
        // Реализуем двунаправленность связи
        this.autopilot.setAutobus(this); // передаю ссылку на этот автобус //private Autobus autobus; строка в автопилоте 12.
    ...
    boolean takePassenger(Passenger passenger) {
        ...
        if (countPassengers < capacity) {      // проверяем есть ли места в автобусе
            // Место есть
            // Надо проверить, есть ли такой пассажир в автобусе
            // Декомпозиция
            if (isPassengerInBus(passenger)) {  // создаём метод isPassengerInBus -   Alt + Enter  на 125 строке
    ...
    private boolean isPassengerInBus(Passenger passenger) {
        for (int i = 0; i < countPassengers; i++) {             // перебираем список пассажиров
            if (passengers[i].getId() == passenger.getId()) {   // если id совпали,значит пассажир в автобусе
    ...
    public String toString() {
        return "Autobus: {" +
                ...
                ", autopilot=" + autopilot.toString() +    // автопилот создавали позже, сами теперь добавляем (строка: 75)
                '}';        //Autobus: {id=0 , capacity=15, driver=BusDriver: {id: 101, name: Peter}, autopilot=Autopilot: {SV: 'AP-v001'}}
    }
