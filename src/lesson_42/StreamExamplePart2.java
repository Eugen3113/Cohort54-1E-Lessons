...
//        task15();
    ...
    private static void task16() {
        ...
        List<Integer> integers = Arrays.stream(ints)                             // поток примитивов
                .boxed()                                                         // поток оберток
                // работает автоупаковка int -> Integer
//                .mapToObj(i -> i)
//                .mapToObj(i-> Integer.valueOf(i))
                .collect(Collectors.toList());                                      // преобразует поток оберток в List

        System.out.println(integers.size());

        // Получить из коллекции Integer - массив примитивов
        int[] intArray = integers.stream()                                      // поток оберток
                //автораспаковка
                .mapToInt(i -> i)                                               // поток примитивов
                .toArray();                                                      // преобразует поток примитивов в массив

        System.out.println("массив: " + Arrays.toString(intArray));             // [1, 2, 3, 4, 5, 6, 7]
    ...
    private static void task15() {
        ...
        List<Integer> integers = List.of(5, 4, 14, 56, 32, 25, -1, -10, 0);       // {0, 1, 2 ,56, 76, 88}

        Optional<Integer> first = integers.stream()                                 // поток оберток
                .sorted()
//                .findFirst(); // Optional<T> получить первый элемент потока
                .findAny(); // Optional<T> получить случайны элемент потока

        int min = first.get();                                                        // Optional<T> получить первый элемент потока
    ...
    private static void task14() {
        ...
        // {0, 1, 2 ,56, 76, 88}
        List<Integer> smallest = integers.stream()                                 // поток оберток
                .sorted()                                                          // сортировка в естественном порядке
                .limit(3)                                                // получить первые 3 элемента потока
                .collect(Collectors.toList());                                     // преобразует поток оберток в List
        System.out.println("smallest: " + smallest);                               // [0, 1, 2]

        // Получить список чисел, отбросив 2 самых маленьких
        List<Integer> array = integers.stream()                                     // поток оберток
                .sorted()
                .skip(2)                                                            // отбросить первые 2 элемента потока
                .limit(5)                                                           // получить следующие 5 элементов потока
                .collect(Collectors.toList());                                       // преобразует поток оберток в List
    ...
    private static void task13() {
        ...
        long count = integers.stream()                                                 // поток оберток
                .filter(n -> n % 2 == 0)                                               // отбросить нечетные числа
                .count();                                                               // получить кол-во элементов потока

        System.out.println("Кол-во четных: " + count);
    }

    private static void task12() {
        List<Cat> cats = getListCats();                                                 // Метод getListCats() возвращает List<Cat>.
        ...
        Optional<String> longetNameOptional = cats.stream()               // cats.stream() — превращаем List<Cat> в поток (Stream<Cat>).
                .filter(Objects::nonNull) // проверка null объектов Cat. Отфильтровываем null-значения в List<Cat>.
//                .map(cat ->  cat.getName())
                .map(Cat::getName)                                     // Преобразуем Stream<Cat> в Stream<String> (поток имен котов).
                .filter(Objects::nonNull)                 // Фильтруем null-значения имен котов (например, если cat.getName() == null).
                .max(Comparator.comparing(String::length));             // получить самое длинное имя.  сравниваем по длине имени.
//                .max((name1, name2) -> Integer.compare(name1.length(), name2.length()));
//                .max((name1, name2) -> name1.length() - name2.length());

        if (longetNameOptional.isPresent()) {                            // проверяем, есть ли длинное имя кота в списке котов cats
            System.out.println(longetNameOptional.get());                // получить самое длинное имя
        } else {                                              //Если список пустой или все имена null, выводим "Котов с именами не найдено".
        ...
        String longestString = cats.stream()
                ...
                .orElse("");             // Если список пустой или все имена null, то возвращаем пустую строку "" или null если надо
//                .orElse(null);               //  можно вернуть null, НО  null может вызвать NullPointerException в будущем.
...
//        System.out.println(findMe.getName());
        ...
        Optional<Cat> optionalCat = findOptionalCat("Bear");  // findOptionalCat() возвращает Optional<Cat> или null если ничего не нашли
        ...
        if (optionalCat.isPresent()) {               // если в Optional завернут не-null (там есть значение)
            ...
            System.out.println("Вытащили из optional котика: " + cat.getName());  // Вытащили из optional котика: Bear (там есть значение)
        } else {
            System.out.println("Там завeрнут null");
        }

        // Возвращает значение, если оно присутствует. Если внутри null - вернет defaultValue
        Cat catDefault = optionalCat.orElse(new Cat("Default", 1, "white")); // Возвращает значение, если оно присутствует.
                                                                           // Если внутри null - вернет defaultValue Cat("Default", 1, "white")
    ...
    private static Optional<Cat> findOptionalCat(String name) {   // findOptionalCat() возвращает Optional<Cat> или null
                                                                  // если ничего не нашли (там завернут null)
        List<Cat> cats = getListCats();                             // список кошек с весом больше 4 и именем Bear

        Cat result = null;                                          // переменная для хранения нашего кота
        for (Cat cat : cats) {                                      // проходим по списку кошек и ищем Bear
            if (cat.getName().equals(name)) {                         // если нашли Bear
                result = cat;                                       // запоминаем его в переменную result
            }
        }
        ...
        return Optional.ofNullable(result);                         // возвращает Optional. Можно завернуть или значение, или null
    }

    private static Cat findCat(String name) {                        // findCat() возвращает Cat или null
    ...
    private static void task10() {
        ...
        List<Cat> cats = new ArrayList<>(                               // создание списка кошек
                List.of(cat, cat1, cat2, cat3, cat4, cat)               // добавление кошек
        );

        cats.forEach(System.out::println);                              // вывод списка кошек на экран
        System.out.println();

        cats.stream()                                                    // создание стрима кошек
                .distinct()                                              // удаление дубликатов кошек
                .forEach(System.out::println);                            // вывод списка кошек на экран
    ...
    private static void task9() {
        ...
        Map<String, Integer> map = new HashMap<>();                    // создание Map
        map.put("Apple", 1);
        map.put("Banana", -10);
        map.put("Cherry", 5);

        // Создание стрима из элементов entrySet
        map.entrySet().stream()                                        // создание стрима из элементов entrySet
                .filter(entry -> entry.getValue() > 0)                   // фильтрация по значению (вес более 0)
                .forEach(entry -> System.out.println(entry.getKey() + ": " + entry.getValue()));  // вывод на экран
    ...
    private static void task8() {
        ...
        List<Cat> longCats = Arrays.stream(cats)                         // создание потока кошек
...
//                .filter(c -> Objects.nonNull(c)) // оставить не null значения
                ...
                .filter(c -> c.getName().length() > 4)     // оставить кошек с именем, длиннее 4 символов
...
//        Objects.nonNull(null); // вернет true, если параметр НЕ null
    ...
    private static void task7() {
        List<Cat> cats = getListCats();                                 // создание списка кошек

        // Вывести на экран имена котов, чей вес меньше 5 кг.
        // Вывести на экран котов, после фильтрации

        Stream<String> catStream = cats.stream()                        // создание потока кошек
                .filter(c -> c.getWeight() < 5)                          // оставить кошек, вес которых меньше 5 кг
                .peek(cat -> System.out.println("After filter:" + cat))   // промежуточное действие - показать кошек после фильтрации
                .map(cat -> cat.getName());                              // преобразование потока кошек в поток имен кошек (поток String)
                                                                         // преобразуем в поток String
        /*
        ...
        catStream.forEach(name -> System.out.println(name));             // преобразование потока String в поток кошек
//        catStream.forEach(System.out::println);
    ...
    private static void task6() {
        ...
        Эффективность
         */
        List<String> names1 = cats.stream()                              // создание потока кошек
                .filter(cat -> cat.getName().length() < 5)               // оставить кошек, у которых имена короче 5 символов
                .map(Cat::getName)                                       // преобразование потока кошек в поток имен кошек (поток String)
                                                                         // преобразуем в поток String
                .collect(Collectors.toList());                               // преобразование потока String в список
         ...
         */
        System.out.println("\n=====================");
        List<String> names2 = cats.stream()                              // создание потока кошек
                .map(Cat::getName)                                       // преобразование потока кошек в поток имен кошек (поток String)
                .filter(name -> name.length() < 5)                       // оставить имена, длиннее 4 символов
                .collect(Collectors.toList());                           // преобразование потока String в список
    ...
    private static void task5() {
        ...
        */
        ...
        List<String> names = cats.stream()                                  // создание потока кошек с весом больше 4 кг
                .filter(cat -> cat.getWeight() > 4)                         // оставить кошек с весом больше 4 кг
//                .map(cat -> cat.getName())
                .map(Cat::getName)                                          // преобразование потока кошек в поток имен кошек
                .filter(name -> name.length() > 4)                          // оставить имена, длиннее 4 символов
    ...
    private static void task4() {
        ...
        Stream<String> namesStream = cats.stream()                        // создание потока кошек
                .map(cat -> cat.getName());                               // преобразование потока кошек в поток имен кошек

        List<String> catNames = cats.stream()                             // создание потока кошек
                .map(Cat::getName)                  // преобразование потока кошек в поток имен кошек (поток String) преобразуем в поток String
                .collect(Collectors.toList());                             // собрать в список
...
//        }
    ...
    private static void task3() {
        ...
        Stream<Cat> stream = cats.stream()                          // создание потока кошек
                .filter(c -> c.getName().length() > 4);

        // Пока не запущен терминальный метод - промежуточные методы не выполняются
        List<Cat> longNames = stream.collect(Collectors.toList());  // собрать в список
    ...
    private static void task2() {
        ...
        Stream<Cat> catStream = cats.stream()                        // создание потока кошек
                .filter(cat -> cat.getWeight() > 4);                 // оставить кошек с весом больше 4
...
//        List<Cat> fatCats = catStream.toList(); // Java 17 or higher  .collect(Collectors.toList() = .toList()
    ...
    private static void task1() {
        ...
        result.sort(Comparator.naturalOrder());              // сортировка в естественном порядке
